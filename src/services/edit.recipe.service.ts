...
export class EditRecipeService {
	...
	public set recipe (recipe:Recipe) {
		this._recipe=  new Recipe(recipe.name,recipe.description, recipe.imgPath);
		this._recipe.ingredients=recipe.ingredients.filter(v=> true);
		
		this._new_recipe=new Recipe(recipe.name,recipe.description, recipe.imgPath);
	...
	public edited():boolean{
		return ! (
				...
				this._recipe.description==this._new_recipe.description &&
